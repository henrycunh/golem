    ...
    async function initPersonaList() {
        ...
        if (!personaList.value.length) {
            await createPersona({
                id: nanoid(),
                title: 'chatQuery',
                instructions: 'You are chatQuery, a large language model based assistant. Answer as concisely as possible or generate in on block a python code when it necessary.',
            })
        }
    }
